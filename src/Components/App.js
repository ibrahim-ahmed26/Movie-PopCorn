...
export default function App() {
  ...
  useEffect(function () {
    async function fetchMovies() {
      ...
      const result = await fetch(
        `https://www.omdbapi.com/?i=tt3896198&apikey=${apiKey}&s=interstellar`
      );
  ...
  return (
    <>
      ...
      <Main>
        <Box>{isLoading ? <Loading /> : <ListedMovies movies={movies} />}</Box>
        <Box>
...
function SelectedMovieList({ watched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <li key={movie.imdbID}>
          <MovieHeader movie={movie} />
          <MovieDetailsSelection movie={movie} />
        </li>
      ))}
    </ul>
  );
}
function MovieDetailsSelection({ movie }) {
  return (
    <div>
      <p>
        <span>⭐️</span>
